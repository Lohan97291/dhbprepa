...
type Step =
  ...
  | {
      ...
      title: string;
      icone?: string;
      exos: Array<{ nom: string; detail?: string }>;
      note?: string;
      blockLabel?: string;
    };
...
function buildSteps(seance: SeanceLike): Step[] {
  ...
  for (const b of seance.blocs ?? []) {
    ...
    if (b.isPPP && b.pppExos?.length) {
      out.push({ kind: "ppp", exos: b.pppExos, ...base });
...
export function SeancePlayer({ seance, joueur, weekIdx, sessionIdx, date, onExit }: Props) {
  ...
  async function validate(missed = false) {
    ...
    else toast.success("Séance validée 💪", { description: `RPE ${rpe}/10 enregistré` });
    try {
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    dispatch({
                      ...
                      stepIdx: state.idx,
                      exoIdx: i,
                      exos: step.exos.length,
                    });
                  }}
                  className={`flex w-full items-center gap-4 rounded-2xl border p-4 text-left transition ${
                    done
                      ? "border-[color:var(--draveil)]/50 bg-[color:var(--draveil)]/[0.12]"
                ...
                >
                  <span
                    className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl border-2 transition ${
                        ...
                        ? "border-[color:var(--draveil)] bg-[color:var(--draveil)] text-white"
                        : "border-white/20"
                    }`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {exo.detail && (
                      <div className="mt-1 text-sm text-muted-foreground">{exo.detail}</div>
                    )}
                  </div>
                </button>
              );
...
function FinishScreen({
  ...
  return (
    <div className="mx-auto flex min-h-full max-w-md flex-col px-6 pb-8 pt-10">
      <button
        onClick={onExit}
        className="self-start rounded-full p-2 text-foreground/60 hover:bg-white/5"
        aria-label="Fermer"
      >
        <X className="h-6 w-6" />
      </button>
      ...
      <div className="mt-8">
          ...
          {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
            <button
              ...
              className={`h-11 flex-1 rounded-lg text-sm font-bold transition ${
                rpe >= n ? "text-white shadow-brand" : "bg-white/[0.04] text-muted-foreground"
              }`}
              style={
                rpe >= n
                  ? {
        ...
        <textarea
          ...
          className="mt-5 w-full resize-none rounded-2xl border border-white/8 bg-white/[0.02] p-4 text-sm text-foreground placeholder:text-muted-foreground focus:border-[color:var(--draveil)] focus:outline-none"
          rows={3}
        />
        ...
        <div className="mt-6 flex gap-2">
          <button
            onClick={onMiss}
            disabled={saving}
            className="flex-1 rounded-2xl border border-white/8 bg-white/[0.02] py-4 text-sm font-semibold text-muted-foreground disabled:opacity-40"
          >
            Séance manquée
          </button>
          <button
            ...
            className="flex-[2] rounded-2xl gradient-brand py-4 text-sm font-bold text-white shadow-brand disabled:opacity-40"
          >
