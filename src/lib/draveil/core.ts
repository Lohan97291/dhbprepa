...
// ══════════════════════════════════════════════════════════════════════════════
export function genPhase2IndivSessions(weekIdx, joueur, dernierRpe=null, rythmeChoisi=null){
  // Résoudre le rythme effort/récup
  const rythme = rythmeChoisi
    ? RYTHMES_DISPO.find(r => r.effortSec === rythmeChoisi) || RYTHMES_DISPO[1]
    : (() => {
        const s = getRhythmeSuggere(dernierRpe);
        return { effortSec:s.effortSec, recupSec:s.recupSec };
      })();
